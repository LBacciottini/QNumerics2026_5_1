...
@resumable function entangler(
    ...
    while true
        @yield get(left_memory, is_empty_memory) & get(right_memory, is_empty_memory)

        attempts = rand(attempts_until_success) + 1
        @yield timeout(env, attempts * attempt_time)

        @yield put!(left_memory, EntangledPair(right_node)) & put!(right_memory, EntangledPair(left_node))
    end
end
...
@resumable function swapper(
    ...
    while true
        ...
        left_request = get_left(chain, node, left_remote) & get(left_remote_memory, is_entangled_with(node))
        @yield left_request

        right_request = get_right(chain, node, right_remote) & get(right_remote_memory, is_entangled_with(node))
        @yield right_request

        @yield timeout(env, swap_time)

        @yield put_left!(chain, node, EmptyMemory()) & put_right!(chain, node, EmptyMemory())
        @yield put!(left_remote_memory, EntangledPair(right_remote)) & put!(right_remote_memory, EntangledPair(left_remote))
    end
end
