...
ConcurrentSim.schedule(idea, 42)
...
@resumable function philosopher(env::Environment)
...
sim = Simulation()
process = @process philosopher(sim)

# Run the simulation

run(sim)

# Food for thought. How do we implement a process using events and callbacks?
...
function benchmark_simulation(num_samples)
    matrix_times = logrange(1E-10, 1E10, 100)
