...
# that want to be entangled, and performs the swap once all relevant tickets
# are obtained.
...
@resumable function request_fulfiller(env::Environment, switch::EntanglementSwitch,
    ...
    while true
        ...
        waited_too_long = timeout(env, max_wait_time)
        @yield AllOf(entanglement_tickets...) | waited_too_long

        if state(waited_too_long) == ConcurrentSim.processed  # cutoff was triggered
            #md println("$(now(env)): time out, request $request")
            for evt in entanglement_tickets
                if state(evt) == ConcurrentSim.processed
                    put!(switch.entanglement_tickets, value(evt)) # return unused tickets
                else
                    cancel(switch.entanglement_tickets, evt) # stop waiting for unobtained tickets
                end
            end
        else  # all tickets obtained, now we consume the entanglement
            #md println("$(now(env)): request $request fulfilled")
            for end_node in request # remove the entangled states from the switch
                deleteat!(switch.entanglements,
                    findfirst(isequal(end_node), switch.entanglements))
            end
            put!(switch.qubits, length(request)) # release the qubits
            switch.num_open_requests -= 1 # log that we have fulfilled the request
